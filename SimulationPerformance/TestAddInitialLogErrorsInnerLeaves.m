% test AddInitialLogErrorsInnerLeaves
pdeltas = zeros(7,1);
...
assert(all(AddInitialLogErrorsInnerLeaves(pdeltas,Sampled3Sigma,Sampled2Sigma) == sqrt(pi) * ones(7,1), 'all'))
...
assert(all(AddInitialLogErrorsInnerLeaves(pdeltas,Sampled3Sigma,Sampled2Sigma) == sqrt(pi) * ResultingQubitsWithErrors, 'all'))
...
assert(all(AddInitialLogErrorsInnerLeaves(pdeltas,Sampled3Sigma,Sampled2Sigma) == pdeltasAllCorrErr, 'all'))
