function [Zerr,Xerr, sTotal] = InnerAndOuterLeafs(L, sigGKP, etad, n, k, v, leafs, N)
...
%concatenated GKP + Steane code while the outer leafs being just GKP
%qubits.
...
%           preparation
%leafs -    this parameter specifies whether we run the simulation of both
%           inner and outer leafs, only outer or only inner.
%           0 - only outer leafs
%           1 - full scheme
%           2 - only inner leafs
...
%           during the logical Bell measurement on the inner leafs.
...
%We count the errors on the inner leafs and outer leafs separately.
%For the outer leafs we simulate k parallel multiplexed links. For the
%inner leafs we can simulate just one Bell measurement as all the links are
%equivalent for the inner leafs.
%For the inner leafs we also count the probabilities that the Steane code
...
if leafs == 0
    ...
    parfor i = 1:N
        [logErrInner, s] = InnerLeafs(L, sigGKP, etad, n, ErrProb3Sigma, ErrProb2Sigma);
...
%Simulate outer leafs
if leafs == 2
    ZerrOuter = zeros(k,1);
    XerrOuter = zeros(k,1);
else
    parfor i = 1:N
        logErrOuter = OuterLeafs(L, sigGKP, etad, k, ErrProb3Sigma, ErrProb2Sigma);
...
%error either on the inner or outer leafs, but not both (then they cancel)
