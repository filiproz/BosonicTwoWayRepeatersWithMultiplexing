...
[logErr,s] = InnerLeafs(LPerfect, sigGKPPerfect, etadperfect, n, ErrProb3SigmaPerfect,ErrProb2SigmaPerfect);
...
%cancel.
[logErr,s] = InnerLeafs(LPerfect, sigGKPPerfect, etadperfect, n, ErrProb3SigmaPerfect, ErrProb2SigmaDeterministicError);
assert(all(logErr == [0,0]))
assert(all(s == [0,0]))

%% Test 3: Deterministic 3Sigma errors
%Again the errors from the left cancel with the errors from the right.
[logErr,s] = InnerLeafs(LPerfect, sigGKPPerfect, etadperfect, n, ErrProb3SigmaDeterministicError, ErrProb2SigmaPerfect);
assert(all(logErr == [0,0]));
assert(all(s == [0,0]))

%% Test 4: All deterministic errors
%Again errors cancel from both sides.
[logErr,s] = InnerLeafs(LPerfect, sigGKPPerfect, etadperfect, n, ErrProb3SigmaDeterministicError, ErrProb2SigmaDeterministicError);
assert(all(logErr == [0,0]))
assert(all(s == [0,0]))